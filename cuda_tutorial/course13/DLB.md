  ...
  3. 最朴素的方法: 两阶段/三阶段
    ...
    2. 对aggregate进行prefix_sum(exclusive chunk prefix)
      aggregates_in:          [11, 25, 40, 50]
      offset(aggregates_ex):  [0,  11, 25, 40]
    3. offset加回每个chunk
    缺点:
  ...
  4. 另一种更直觉但不够好: 链式等待
    chunk0做完后, 得到prefix0 = aggregate0 = 11
    chunk1等chunk0, 拿到prefix0, 算出自己的prefix1 = prefix0 + aggregate1 = 25
    chunk2等chunk1, 拿到prefix1, 算出自己的prefix2 = prefix1 + aggregate2 = 40
    chunk3等chunk2, 拿到prefix2, 算出自己的prefix3 = prefix2 + aggregate3 = 50
    缺点: 依赖性过强
    prefixi = prefixi-1 + aggregatei
    prefixi = aggregate0 + ... + aggregatei
    offseti = prefixi-1
    offseti = aggregate0 + ... + aggregatei-1
  ...
  6. 用CPU真正走一遍DLB
    ...
    6.3 chunk2(真正体现DLB的地方)
      假设一个时序
        ...
        如果链式, 只能等待inclusive_prefix(chunk1)(强依赖)
        DLB
          1. chunk1
            发现
          ...
          3. 求解
            ...
            exclusive_prefix(chunk2) = inclusive_prefix(chunk0) + aggregate1
            offset2 = aggregate0 + aggregate1
          虽然chunk1的完整prefix还没出来(inclusive_prefix(chunk1): 未知)
          但是chunk2已经知道自己的offset2, 且立刻发布inclusive_prefix(chunk2)
    ...
    6.4 chunk3
      ...
      6.4.2 假设一个时序
        ...
        chunk2: AGGREGATE_READY, aggregate2 = 15
        往前看, 看chunk2, aggregate2 = 15
        看chunk1, inclusive_prefix(chunk1) = 25
        exclusive_prefix(chunk3) = inclusive_prefix(chunk2)(还没发布)
          inclusive_prefix(chunk2) = exclusive_prefix(chunk2) + aggregate2
