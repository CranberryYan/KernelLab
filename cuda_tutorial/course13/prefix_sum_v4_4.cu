...
__global__ void prefix_sum_inclusive_dlb_optimized(
  ...
  for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
    // 1. 优化点
    //  虽然tid的每个循环是连续的, 但是同一个warp之间, 是稀疏的
    // 假设i = 0, tile_base = 0
    //  tid0:   idx: 0    addr: 0x0000
    //  tid1:   idx: 4    addr: 0x0010
    //  ...
    //  tid31:  idx: 124  addr: 0x01F0
    // 一次循环, 需要load 32 * 4 = 128B, 地址分散在0x0000~0x01F0, 覆盖500B的窗口
    // 一次warp load指令总共只请求128B有效数据, 需要load 4次才行
...
int main() {
  ...
  printf("v4.4 benchmark (single-kernel DLB)\n");
