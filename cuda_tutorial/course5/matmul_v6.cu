...
__device__ void load_gmem_to_reg(int N, int K,
																 const float* A, const float* B,
																 float4* A_reg_vec, float4* B_reg_vec,
																 int a_vec_x, int a_vec_y,
																 int b_vec_x, int b_vec_y) {
  #pragma unroll
	for (uint row = 0; row + a_row_stride <= BM; row += a_row_stride) {
		int vec_index = row / a_row_stride;
		A_reg_vec[vec_index] =
				*reinterpret_cast<const float4*>(&A[(a_vec_y + row) * K + a_vec_x * 4]);
	}

  #pragma unroll
	for (uint row = 0; row + b_row_stride <= BK; row += b_row_stride) {
		int vec_index = row / b_row_stride;
		B_reg_vec[vec_index] =
...
template <const int BM, const int BN, const int BK,
          const int a_row_stride, const int b_row_stride>
__device__ void store_reg_to_smem(float* Asmem, float* Bsmem,
																 float4* A_reg_vec, float4* B_reg_vec,
																 int a_vec_x, int a_vec_y,
																 int b_vec_x, int b_vec_y) {
	#pragma unroll
  for (uint row = 0; row + a_row_stride <= BM; row += a_row_stride) {
		int vec_index = row / a_row_stride;
		const float4 tmp = A_reg_vec[vec_index];
  ...
  for (uint row = 0; row + b_row_stride <= BK; row += b_row_stride) {
    ...
    *reinterpret_cast<float4*>(
        &Bsmem[(b_vec_y + row) * BN + b_vec_x * 4]) = B_reg_vec[vec_index];
  }
}
...
// v6
// prologue:
//	load tile0: gmem -> smem[0]
// main loop:
//	prefetch tile1: gmem -> reg
//	compute tile0 from smem[0]
//	load tile1: reg -> smem[1]
//	sync

//	prefetch tile2: gmem -> reg
//	compute tile1 from smem[1]
//	load tile2: reg -> smem[0]
//	sync
...
__global__ void __launch_bounds__(NUM_THREADS) mysgemm_v6(
  ...
  constexpr uint vec_len = 4;
	...
	constexpr uint a_row_stride =
			(NUM_THREADS * vec_len) / BK; // 一个block, 一次搬几行
	constexpr uint A_VEC_PER_THREAD =
			BM * BK / (NUM_THREADS * vec_len);

	constexpr uint b_vec_num_x = BN / vec_len;
	const uint b_vec_x = tid % b_vec_num_x;
	const uint b_vec_y = tid / b_vec_num_x;
	constexpr uint b_row_stride = (NUM_THREADS * vec_len) / BN;
	constexpr uint B_VEC_PER_THREAD =
			BK * BN / (NUM_THREADS * vec_len);

	float thread_results[thread_tile_ele_num] = {0.0f};
	float reg_m[thread_tile_m] = {0.f};
	float reg_n[thread_tile_n] = {0.f};
	float4 A_reg_vec[A_VEC_PER_THREAD];
	float4 B_reg_vec[B_VEC_PER_THREAD];

	// prologue: load tile 0
	load_gmem_to_reg<BM, BN, BK, a_row_stride, b_row_stride>(
		N, K,
		A, B,
		A_reg_vec, B_reg_vec,
		a_vec_x, a_vec_y,
	...
	store_reg_to_smem<BM, BN, BK, a_row_stride, b_row_stride>(
		Asmem[0], Bsmem[0],
		A_reg_vec, B_reg_vec,
		a_vec_x, a_vec_y,
		b_vec_x, b_vec_y);
  ...
  for (uint k = BK; k < K; k += BK) {
		// step 1: prefetch next tile: gmem -> reg
		load_gmem_to_reg<BM, BN, BK, a_row_stride, b_row_stride>(
      N, K,
      A + k, B + k * N,
      A_reg_vec, B_reg_vec,
      ...
      b_vec_x, b_vec_y);
		...
		// step 2: compute current tile from smem[PingPong]
		process_from_smem<BM, BN, BK, WM, WN,
											WMITER, WNITER, WSUBM, WSUBN, TM, TN>(
				reg_m, reg_n, thread_results,
				Asmem[PingPong], Bsmem[PingPong],
				warp_x, warp_y,
				subtile_thread_row, subtile_thread_col);

		// step 3: write prefetched next tile into smem[!PingPong]
		store_reg_to_smem<BM, BN, BK, a_row_stride, b_row_stride>(
				Asmem[!PingPong], Bsmem[!PingPong],
				A_reg_vec, B_reg_vec,
				a_vec_x, a_vec_y,
...
int main() {
  ...
  // 打开CSV文件
  std::ofstream csv_file("./course5/sgemm_benchmark_v6.csv");
    ...
    try {
      ...
      for (int i = 0; i < warpup_time; ++i) {
        checkCublasError(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N,
                                     &alpha, d_B, N, d_A, N, &beta, d_C_v6, N),
                         "cublasSgemm failed");
      }
      ...
      // 拷贝 cuBLAS 结果
      checkCudaError(cudaMemcpy(C_cublas, d_C_v6, size, cudaMemcpyDeviceToHost),
                     "cudaMemcpy C_cublas failed");

      // mysgemm_v6
      checkCudaError(cudaMemset(d_C_v6, 0, size), "cudaMemset d_C_v6 failed");

      const uint v6_NUM_THREADS = 256;
      const uint v6_BN = 128;
      const uint v6_BM = 128;
      const uint v6_BK = 16;
      const uint v6_WN = 32;
      const uint v6_WM = 64;
      const uint v6_WNITER = 2;
      const uint v6_TN = 8;
      const uint v6_TM = 4;
      dim3 blockDim(v6_NUM_THREADS);

      constexpr uint NUM_WARPS = v6_NUM_THREADS / 32;

      // warptile in threadblocktile
      static_assert((v6_BN % v6_WN == 0) and (v6_BM % v6_WM == 0));
      static_assert((v6_BN / v6_WN) * (v6_BM / v6_WM) == NUM_WARPS);
      // threads in warpsubtile
      static_assert(
          (v6_WM * v6_WN) % (WARP_SIZE * v6_TM * v6_TN * v6_WNITER) == 0);
      constexpr uint v6_WMITER =
          (v6_WM * v6_WN) / (32 * v6_TM * v6_TN * v6_WNITER);
      // warpsubtile in warptile
      static_assert((v6_WM % v6_WMITER == 0) and (v6_WN % v6_WNITER == 0));

      static_assert(
          (v6_NUM_THREADS * 4) % v6_BK == 0,
          "NUM_THREADS*4 must be multiple of v6_BK to avoid quantization "
          "issues during GMEM->SMEM tiling (loading only parts of the "
          "final row of Bs during each iteraion)");
      static_assert(
          (v6_NUM_THREADS * 4) % v6_BN == 0,
          "NUM_THREADS*4 must be multiple of v6_BN to avoid quantization "
          "issues during GMEM->SMEM tiling (loading only parts of the "
      ...
      dim3 gridDim(CEIL_DIV(N, v6_BM), CEIL_DIV(N, v6_BN));

      for (int i = 0; i < warpup_time; ++i) {
        mysgemm_v6<v6_BM, v6_BN, v6_BK, v6_WM, v6_WN, v6_WNITER,
                   v6_TM, v6_TN, v6_NUM_THREADS>
            <<<gridDim, blockDim>>>(N, N, N, alpha, d_A, d_B, beta, d_C_v6);
      }
      cudaDeviceSynchronize();

      checkCudaError(cudaEventRecord(start),
                     "cudaEventRecord(start v6) failed");
      for (int i = 0; i < repeat_time; ++i) {
        mysgemm_v6<v6_BM, v6_BN, v6_BK, v6_WM, v6_WN, v6_WNITER,
                   v6_TM, v6_TN, v6_NUM_THREADS>
            <<<gridDim, blockDim>>>(N, N, N, alpha, d_A, d_B, beta, d_C_v6);
      }
      checkCudaError(cudaEventRecord(stop), "cudaEventRecord(stop v6) failed");
      checkCudaError(cudaEventSynchronize(stop),
                     "cudaEventSynchronize v6 failed");
      checkCudaError(cudaGetLastError(), "cuda get last error failed");
      float v6_time = 0;
      checkCudaError(cudaEventElapsedTime(&v6_time, start, stop),
                     "cudaEventElapsedTime v6 failed");

      // 拷贝手写 kernel 结果
      checkCudaError(cudaMemcpy(C_v6, d_C_v6, size, cudaMemcpyDeviceToHost),
                     "cudaMemcpy C_v6 failed");
      // 结果比较
      int error_count = 0;
      ...
      float v6_gflops =
          repeat_time * 2.0f * N * N * N / (v6_time * 1e6f); // GFlops

      float ratio = v6_gflops / cublas_gflops;
      // 写入CSV
      csv_file << N << "," << cublas_gflops << "," << v6_gflops << ","
