...
__global__ void transpose_naive(half *input, half *output,
...
class Tester {
public:
explicit Tester(size_t M = 512, size_t N = 2048, size_t K = 1024,
								size_t warmup_iterations = 1,
								size_t profiling_iterations = 10,
...
void evaluate(Func &&hgemm, const std::string &name) {
	...
	m_warmup_time = static_cast<double>(
		m_cuda_timer.end()) / static_cast<double>(m_warmup_iterations);
...
private:
void cublas_tensor_op(half *A, half *B, half *C,
											size_t M, size_t N, size_t K) {

	// 创建 cuBLAS 句柄, 几乎所有 cuBLAS 调用都需要这个 handle
	cublasHandle_t handle = nullptr;
	HGEMM_CHECK_CUBLAS_ERROR(cublasCreate(&handle));

	// CUBLAS_TENSOR_OP_MATH: 使能 tensor_core
	// CUBLAS_DEFAULT_MATH: 关闭 tensor_core
...
void profile(Func &&hgemm, const std::string &name) {
	...
	m_profiling_time =
		static_cast<double>(m_cuda_timer.end()) /
			static_cast<double>(m_profiling_iterations);
	m_throughput =
			static_cast<double>(m_M * m_N * m_K * 2) * 1e-12 /
	...
	HLOG("%s exit, \
		profiling time: %.3f ms (%.2f%%), throughput: %.3f TFLOPS (%.2f%%)",
		name.c_str(),
	...
	// row major, M * N, base result, init matrix C before each hgemm
	std::shared_ptr<Matrix> m_base = nullptr;
