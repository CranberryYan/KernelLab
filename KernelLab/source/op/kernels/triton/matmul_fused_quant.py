...
# Cq = Quantize_fp8(Dequantize_fp8(Aq, Sa) × Dequantize_fp8(Wq, Sw))
# Aq和Wq是量化后的输入和权重, 均为fp8
# Sa和Sw是量化参数, 均为fp32
# 先反量化将Aq和Wq: fp8 -> fp32 -> 进行计算 -> 量化 -> fp32 -> fp8
...
@triton.jit
def fp8_gemm_kernel(x_ptr, w_ptr, y_ptr,
                    x_s_ptr, w_s_ptr,
                    M,
                    N: tl.constexpr,
                    K: tl.constexpr,
...
def fp8_gemm_host(x: torch.Tensor, x_s: torch.Tensor,
  ...
  fp8_gemm_kernel[grid](x, w, y, x_s, w_s, M, N, K)
...
def test_fp8_gemm():
  ...
  y_fp8 = fp8_gemm_host(x_dequant, one_x, w_dequant, one_w)

  y_ref = x @ w.T

  max_error = (y_ref - y_fp8).abs().max().item()
  mse = ((y_ref - y_fp8) ** 2).mean().item()
