...
__global__ void matmul_kernel(const float* input, const float* weight,
                              float* output, int M, int K) {
  // smem_size: THREAD_PER_BLOCK * sizeof(float) = 512Bytes
...
__global__ void matmul_kernel_cu_fp32int8(const float* input,
  ...
  int start_row = blockIdx.x;
  int end_row = blockIdx.x + ROW_PER_BLOCK;
  if (start_row >= K) {
    return;
  }
...
void matmul_kernel_cu(const tensor::Tensor& input,
  ...
  if (config && config->stream) {
    matmul_kernel<128, 1><<<K, 128, 0, config->stream>>>(
      input.ptr<float>(), weight.ptr<float>(),
      const_cast<float*>(output.ptr<float>()), M, K);
  } else {
    matmul_kernel<128, 1><<<K, 128>>>(
      input.ptr<float>(), weight.ptr<float>(),
      const_cast<float*>(output.ptr<float>()), M, K);
  }
}
