...
base::Status RoPELayer::checkArgs() const {
  auto status = check_tensor_with_dim(get_input(2),
  ...
  status = check_tensor_with_dim(get_input(1), 
                                 device_type_, data_type_, kv_dim_);
  if (!status) {
