...
BaseLayer::BaseLayer(base::DeviceType device_type,
  ...
  data_type_(data_type), layer_name_(std::move(layer_name)) { }
...
base::Status LayerParam::set_weight(int32_t idx,
  ...
  if (!is_quant_layer_) {
    ...
    // 使用外部传入的scale, 无需alloc
    scales_ = tensor::Tensor(base::DataType::kDataTypeFp32,
                             scale_nums,
                             false,
                             nullptr,
