...
Tensor::Tensor(base::DataType data_type,
               ...
               bool need_alloc,
               std::shared_ptr<base::DeviceAllocator> alloc,
               void* ptr) : dims_(std::move(dims)), data_type_(data_type) {
...
void Tensor::to_cuda(cudaStream_t stream) {
  ...
  if (device_type == base::DeviceType::kDeviceUnknown) {
    ...
    std::shared_ptr<base::CUDADeviceAllocator> cu_alloc =
      base::CUDADeviceAllocatorFactory::get_instance();
    std::shared_ptr<base::Buffer> cu_buffer =
      std::make_shared<base::Buffer>(total_size, cu_alloc);
    cu_alloc->memcpy(buffer_->ptr(), cu_buffer->ptr(), total_size,
...
void Tensor::to_cpu() {
  ...
  if (device_type == base::DeviceType::kDeviceUnknown) {
    ...
    size_t total_size = this->total_size();
    std::shared_ptr<base::CPUDeviceAllocator> cpu_alloc =
      base::CPUDeviceAllocatorFactory::get_instance();
    std::shared_ptr<base::Buffer> cpu_buffer =
      std::make_shared<base::Buffer>(total_size, cpu_alloc);
    cpu_alloc->memcpy(buffer_->ptr(), cpu_buffer->ptr(), total_size,
...
bool Tensor::assign(std::shared_ptr<base::Buffer> buffer) {
  ...
  size_t total_size = this->total_size();
  if (total_size > buffer->total_size()) {
...
bool Tensor::allocate(std::shared_ptr<base::DeviceAllocator> allocator,
  ...
  if (buffer_ && total_size <= buffer_->total_size()) {
    if (!need_realloc) {
      return true;
    }
  }

  buffer_ = std::make_shared<base::Buffer>(total_size, allocator,
                                           nullptr, false);
  if (!buffer_->ptr()) {
...
Tensor Tensor::clone() const {
  ...
  new_tensor.buffer_ = std::make_shared<base::Buffer>(total_size, allocator);
